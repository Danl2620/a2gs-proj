...
;; DrawString(A = u16ptr LenString, X = u16 ScreenOffset)
DrawString    sta ]F_StrPtr  		;; *(]F_StrPtr) = LenString
              stx ]F_CurrentPos 	;; *(]F_CurrentPos) = ScreenOffset
              stz ]F_CharIdx 		;; *(]F_CharIdx) = 0
              lda (]F_StrPtr)		;; A = *(]F_StrPtr) -- get length byte
...
;; :drawChar(X = u16 TopLeft, Y = u16 CharTableOffset)
:drawChar     lda FontTable,y             ;get real address of char data
			  ...
			  MASK FontData+ 0;#0+DHR_SCREEN
			  MASK FontData+ 2;#2+DHR_SCREEN
			  MASK FontData+ 4;#SCREEN_WIDTH    +DHR_SCREEN
			  MASK FontData+ 6;#SCREEN_WIDTH  +2+DHR_SCREEN
			  MASK FontData+ 8;#SCREEN_WIDTH*2  +DHR_SCREEN
              MASK FontData+10;#SCREEN_WIDTH*2+2+DHR_SCREEN
              MASK FontData+12;#SCREEN_WIDTH*3  +DHR_SCREEN
              MASK FontData+14;#SCREEN_WIDTH*3+2+DHR_SCREEN
              MASK FontData+16;#SCREEN_WIDTH*4  +DHR_SCREEN
              MASK FontData+18;#SCREEN_WIDTH*4+2+DHR_SCREEN
              MASK FontData+20;#SCREEN_WIDTH*5  +DHR_SCREEN
              MASK FontData+22;#SCREEN_WIDTH*5+2+DHR_SCREEN
              rts
